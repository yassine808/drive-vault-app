...
function register(opts: JobsRegisterOpts) {
  ...
  ipcMain.handle(
    "jobs:save",
    requireAuth(async (_e, { job }: { job: Job }) => {
      logger.ipcLog("jobs:save", "Saving job", {
        jobId: job?.id,
        company: job?.company,
      });
      try {
        if (!job || typeof job !== "object") {
          logger.warn("jobs:save", "Invalid job data");
          return { ok: false, error: "Invalid job data" };
        }
        job.company = sanitizeStr(job.company);
        job.role = sanitizeStr(job.role);
        if (job.email && !validEmail(job.email)) {
          logger.warn("jobs:save", "Invalid email", { email: job.email });
        ...
        logger.success("jobs:save", "Job saved", {
          jobId: id,
          company: job.company,
        });
        return { ok: true, id };
  ...
  ipcMain.handle(
    "jobs:reorder",
    requireAuth(async (_e, { jobs }: { jobs: Array<{ id?: string }> }) => {
  ...
  ipcMain.handle(
    ...
    requireAuthNoArgs(async () => {
      ...
      try {
        ...
        logger.success("jobs:trash:load", "Job trash loaded", {
          count: items.length,
        });
        return { ok: true, items };
      } catch (e: unknown) {
        const err = e as Error;
        logError("jobs:trash:load", err);
        return { ok: false, error: err.message };
      }
    }),
  );
  ...
  ipcMain.handle(
    ...
    requireAuth(async (_e, { id }: { id: string }) => {
      ...
      try {
        ...
        const err = e as Error;
        logError("jobs:trash:restore", err);
        return { ok: false, error: err.message };
      }
    }),
  );
