...
export class DriveClient {
    ...
    for (const item of queue) {
      ...
      try {
        ...
        if (item.retryCount < this.MAX_RETRIES) {
          ...
          this.logger.error("drive:sync", `Dropping item after ${this.MAX_RETRIES} retries`, {
            itemId: item.id,
          });
        }
      }
  ...
  private async migrateFlatFiles(): Promise<void> {
    ...
    for (const f of files) {
      ...
      if (parsed) {
        ...
        await this.migrateFileToSubfolder(f, subfolderName, `Moved ${name} → ${subfolderName}/`);
        continue;
      }
      if (name === SETTINGS_FILE_NAME || name === TWOFA_FILE_NAME) {
        await this.migrateFileToSubfolder(f, SUBFOLDERS.settings, `Moved ${name} → settings/`);
        continue;
      }
  ...
  loadItems(type: ItemType): CacheItem[] {
    const arr = this.getCacheArray(type);
    return arr.filter((i) => !i.deletedAt).sort((a, b) => a.sortOrder - b.sortOrder);
  }
   ...
   */
  updateSortOrder(type: DeletableType, items: Array<{ id?: string }>): void {
    const arr = this.getCacheArray(type);
