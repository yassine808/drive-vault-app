...
function register(
  ...
  ipcMain.handle('accounts:save', requireAuthNoArgs(async () => {
    ...
    try {
      ...
      logger.success('accounts:save', 'Account saved', { email: session.email });
      return { ok: true };
    } catch (e: unknown) {
      const err = e as Error;
  ...
  ipcMain.handle('accounts:remove', requireAuthNoArgs(async () => {
    ...
    try {
      ...
      writeAccounts(accounts);
      logger.success('accounts:remove', 'Account removed', { email: session.email });
      return { ok: true };
    } catch (e: unknown) {
      const err = e as Error;
