...
function register(
  ...
  function dbSaveTotp(item: TotpItem, encKey: string): string {
    logger.dbLog('dbSaveTotp', 'Saving TOTP item', { itemId: item?.id, name: item?.name });
    if (!driveClient) throw new Error('Drive not initialized');
    const encData = enc({ name: item.name, issuer: item.issuer, secret: item.secret, icon: item.icon }, encKey);
  ...
  ipcMain.handle('totp:delete', requireAuth(async (_e, { id }: { id: string }) => {
