...
const LEVELS: Record<string, number> = { DEBUG: 0, INFO: 1, SUCCESS: 2, WARN: 3, ERROR: 4, AUTH: 5, IPC: 6, DB: 7 };
...
function write(level: number, ctx: string, msg: string, data?: unknown): void {
  ...
  line += '\n';
  try { fs.appendFileSync(fileForLevel(levelName), line); } catch { /* noop */ }
  try { fs.appendFileSync(path.join(LOG_DIR, 'all.log'), `[${levelName}] ${line}`); } catch { /* noop */ }
}

function writeError(ctx: string, err: unknown): void {
  init();
  const e = err as { message?: string; code?: string | number; stack?: string; response?: { data?: unknown; status?: number } };
  ...
  try { fs.appendFileSync(fileForLevel('ERROR'), line); } catch { /* noop */ }
}

function debug(ctx: string, msg: string, data?: unknown): void { write(LEVELS.DEBUG, ctx, msg, data); }
function info(ctx: string, msg: string, data?: unknown): void { write(LEVELS.INFO, ctx, msg, data); }
function success(ctx: string, msg: string, data?: unknown): void { write(LEVELS.SUCCESS, ctx, msg, data); }
function warn(ctx: string, msg: string, data?: unknown): void { write(LEVELS.WARN, ctx, msg, data); }
function error(ctx: string, msg: string, data?: unknown): void { write(LEVELS.ERROR, ctx, msg, data); }
function authLog(ctx: string, msg: string, data?: unknown): void { write(LEVELS.AUTH, ctx, msg, data); }
function ipcLog(ctx: string, msg: string, data?: unknown): void { write(LEVELS.IPC, ctx, msg, data); }
function dbLog(ctx: string, msg: string, data?: unknown): void { write(LEVELS.DB, ctx, msg, data); }
...
function getLogDir(): string { return LOG_DIR; }
function getLogPath(levelName: string): string { return fileForLevel(levelName); }

export {
  init, ts,
  debug, info, success, warn, error,
  authLog, ipcLog, dbLog,
  writeError,
