// @ts-nocheck — Renderer is bundled by esbuild; main process types are strict

interface AppState {
  user: (UserProfile & { isAdmin?: boolean }) | null;
  passwords: VaultItem[];
  notes: VaultItem[];
  trash: (VaultItem & { _type: string; _deletedAt: string })[];
  jobs: Job[];
  totp: TotpItem[];
...
function toast(msg: string, ms?: number): void { if(ms===undefined) ms=S.settings.toast_duration||2400; logInfo('ui', 'Toast: ' + msg); const el=document.getElementById('toast')!; el.textContent=msg;el.classList.add('show');setTimeout(()=>el.classList.remove('show'),ms);}
function show(id: string): void {document.getElementById(id)!.hidden = false;}
function hide(id: string): void {document.getElementById(id)!.hidden = true;}
  function screen(s: string): void { ['s-login','s-2fa','s-lock','s-app'].forEach(id => { const el = document.getElementById(id); if (el) el.hidden = id !== s; }); }
function clearAllInputs(): void {document.querySelectorAll('input:not([type=checkbox]):not([type=range]),textarea').forEach(el=>{(el as HTMLInputElement | HTMLTextAreaElement).value = '';});}
...
function playTone(freq: number, type: string = 'sine', dur: number = 0.15, vol: number = 0.18, delay: number = 0): void {
  try{const ctx=getACtx();const now=ctx.currentTime+delay;const osc=ctx.createOscillator();const gain=ctx.createGain();
  osc.type = type as OscillatorType;osc.frequency.setValueAtTime(freq,now);gain.gain.setValueAtTime(0,now);
...
const TONES: Record<string, { freqs: number[]; type: string; dur: number; vol: number; gap: number }> = {
...
function logDebug(ctx: string, msg: string): void { /* noop */ }
...
// ═══ CONFIRM ══════════════════════════════════════════════════════════════════
function confirm(opts: { title: string; msg: string; icon?: string; okLabel?: string; okClass?: string; onOk: () => void }): void {
...
(document.getElementById('confirm-overlay') as HTMLElement).addEventListener('click',e=>{if(e.target===(document.getElementById('confirm-overlay') as HTMLElement)){ hide('confirm-overlay'); logInfo('ui', 'Confirm dialog dismissed (overlay click)'); }});
...
function disarmLock(): void { clearTimeout(lockTimer); clearInterval(lockTick); const row = document.getElementById('lock-row') as HTMLElement; if (row) row.hidden = true; }
let _lockInProgress: boolean = false;
...
['mousemove','keydown','mousedown','touchstart'].forEach(ev => document.addEventListener(ev, () => { if (S.user && S.settings.lock_timeout > 0) armLock(); }, { passive: true }));
...
(document.getElementById('btn-login') as HTMLButtonElement).addEventListener('click',async()=>{
  ...
  if(!r.ok){const err=document.getElementById('login-err') as HTMLElement;err.hidden=false;err.textContent=r.error;logErr('auth', 'Login failed', r.error);btn.textContent='Sign in with Google';btn.disabled=false;return;}
...
(document.getElementById('btn-verify2fa') as HTMLButtonElement).addEventListener('click',async()=>{
  ...
  if(!r.ok){(document.getElementById('twofa-err') as HTMLElement).hidden=false;(document.getElementById('twofa-err') as HTMLElement).textContent=r.error;logWarn('auth', '2FA verify failed', r.error);return;}
  if(r.token)window.__vaultToken.set(r.token);
  S.user=r.user;loadVault(r.vault);await loadSettings();enterApp();
  logOk('auth', '2FA verified, login complete');
});
(document.getElementById('twofa-code') as HTMLInputElement).addEventListener('keydown',e=>{if(e.key==='Enter')(document.getElementById('btn-verify2fa') as HTMLButtonElement).click();});
...
function loadVault(v: { passwords?: VaultItem[]; notes?: VaultItem[] } | null | undefined): void {S.passwords=v?.passwords||[];S.notes=v?.notes||[];logInfo('vault', 'Vault loaded into memory', { passwords: S.passwords.length, notes: S.notes.length });}
...
// ═══ TABS ══════════════════════════════════════════════════════════════════════
const _tabCache: Record<string, boolean> = {}; // tracks whether a tab has been loaded at least once
document.querySelectorAll('.nav-btn[data-tab]').forEach(btn => { const b = btn as HTMLElement; b.addEventListener('click', () => switchTab(b.dataset.tab)); });
function switchTab(tab: string): void {
  ...
  document.querySelectorAll('.nav-btn[data-tab]').forEach(b => { const el = b as HTMLElement; el.classList.toggle('active', (el as HTMLElement).dataset.tab === tab); });
...
function updateCounts(): void {
  (document.getElementById('cnt-pw') as HTMLElement).textContent=S.passwords.length;
  (document.getElementById('cnt-notes') as HTMLElement).textContent=S.notes.length;
  (document.getElementById('cnt-trash') as HTMLElement).textContent=S.trash.length;
  (document.getElementById('cnt-jobs') as HTMLElement).textContent=S.jobs.length;
  (document.getElementById('cnt-totp') as HTMLElement).textContent=S.totp.length;
}
...
function renderPasswords(): void {
  ...
  list.forEach(pw=>{
    ...
    eyeBtn.addEventListener('touchstart', (e: TouchEvent) => { e.preventDefault();hidSpan.hidden=true;revSpan.hidden=false;smWrap.hidden=false;updateInlineSm(smWrap,pw.password||'');},{passive:false});
...
(document.getElementById('modal-overlay') as HTMLElement).addEventListener('click',e=>{if(e.target===(document.getElementById('modal-overlay') as HTMLElement))hide('modal-overlay');});
...
function openNote(id: string): void {
  S.activeNote=id;const note = S.notes.find(n => String(n.id) === id);if(!note)return;
  ...
  const autoSave=async()=>{
    ...
    renderNotesList();(document.getElementById('n-status') as HTMLElement).textContent='Saving…';
...
async function loadAndRenderTrash(): Promise<void> {
  ...
  (wrap.querySelector('.trash-loading') as HTMLElement | null)?.remove();
...
(document.getElementById('jobs-search') as HTMLInputElement).addEventListener('input',renderJobsTable);
...
document.querySelectorAll('.sortable').forEach(th =>{
  th.addEventListener('click',()=>{
    ...
    else{S.jobSort.col=t(h as HTMLElement).dataset.col;S.jobSort.dir=1;}
...
function renderJobsTable(): void {
  ...
  const mkStat = function(cls: string, num: number, lbl: string): HTMLElement { const d = document.createElement('div'); d.className = 'job-stat ' + cls; const s = document.createElement('span'); s.textContent = String(num); d.appendChild(s); const l = document.createElement('small'); l.textContent = lbl; d.appendChild(l); return d; };
  ...
  list.forEach(job=>{
    ...
    const _ceb = tr.querySelector('.copy-email-btn') as HTMLButtonElement | null; if (_ceb) _ceb.onclick = e => { e.stopPropagation();navigator.clipboard.writeText(job.email||'');toast('Email copied!');logInfo('jobs', 'Email copied', { company: job.company });};
    ...
    tr.querySelector('.job-status-cell')!.addEventListener('click', (e: MouseEvent) => {
      e.stopPropagation();
      _statusPopupJob=job;
    ...
    const _djb = tr.querySelector('.del-job-btn') as HTMLButtonElement | null; if (_djb) _djb.onclick = () => confirm({
...
(document.getElementById('job-overlay') as HTMLElement).addEventListener('click',e=>{if(e.target===(document.getElementById('job-overlay') as HTMLElement))hide('job-overlay');});
...
(document.getElementById('totp-ok') as HTMLButtonElement).addEventListener('click',async()=>{
  ...
  const item={id:_totpEdit?.id,name,issuer:(document.getElementById('t-issuer') as HTMLInputElement).value.trim(),secret,icon:(document.getElementById('t-icon') as HTMLInputElement).value||'🔐'};
  hide('totp-overlay');
...
(document.getElementById('totp-overlay') as HTMLElement).addEventListener('click',e=>{if(e.target===(document.getElementById('totp-overlay') as HTMLElement))hide('totp-overlay');});

// ═══ MONITOR with circle gauges ═══════════════════════════════════════════════
let _monitorEntries: import('./src/types').LogEntry[] = [];
let _monitorFilter: string = 'all';
let _monitorRefreshTimer: ReturnType<typeof setTimeout> | null = null;

function fmtSize(n: number): string {return n>=1048576?(n/1048576).toFixed(1)+' MB':n>=1024?(n/1024).toFixed(1)+' KB':n+' B';}
...
async function loadMonitor(): Promise<void> {
  ...
  if(sr.ok){
    ...
    (document.getElementById('dash-storage') as HTMLElement).textContent=fmt(st.dbSizeBytes||0);
  ...
  if(isAdmin()){
    (document.getElementById('admin-dashboard') as HTMLElement).hidden=false;
    loadAdminDashboard();
  } else {
    (document.getElementById('admin-dashboard') as HTMLElement).hidden=true;
  }
  ...
  _monitorRefreshTimer=setTimeout(()=>{
    const tab=document.getElementById('tab-monitor') as HTMLElement;
...
async function loadAdminDashboard(): Promise<void> {
  ...
  if(statsRes.ok){
    ...
    (document.getElementById('admin-total-users') as HTMLElement).textContent=st.totalUsers;
    (document.getElementById('admin-total-items') as HTMLElement).textContent=st.totalItems;
    (document.getElementById('admin-total-jobs') as HTMLElement).textContent=st.totalJobs;
    (document.getElementById('admin-total-totp') as HTMLElement).textContent=st.totalTotp;
  }
...
function applyAccent(name: string): void {
  ...
  document.querySelectorAll('.accent-swatch').forEach(s => { const el = s as HTMLElement; el.classList.toggle('active', el.dataset.accent === name); });
}
...
function doGenerate(): string {
  ...
  const l=document.getElementById('gen-slabel') as HTMLElement;if(l){l.textContent=lbl;l.className='slabel '+cls.replace('sl-','s');}
...
function openGen(fillMode: boolean = false): void {
  logInfo('generator', 'Generator opened', { fillMode });
  (document.getElementById('gen-len') as HTMLInputElement).value = S.settings.gen_length || 20;
  (document.getElementById('gen-len-val') as HTMLElement).textContent = S.settings.gen_length || 20;
...
(document.getElementById('btn-2fa') as HTMLButtonElement).addEventListener('click',async()=>{
  ...
  const okBtn=document.getElementById('twofa-ok') as HTMLButtonElement;const disBtn=document.getElementById('twofa-disable') as HTMLButtonElement;
  if(r.enabled){
...
(document.getElementById('twofa-overlay') as HTMLElement).addEventListener('click',e=>{if(e.target===(document.getElementById('twofa-overlay') as HTMLElement))hide('twofa-overlay');});
