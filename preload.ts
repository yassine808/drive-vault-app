...
contextBridge.exposeInMainWorld("api", {
  ...
  save: (type: string, item: Record<string, unknown>) => {
    bridgeLog("call", "vault:save", true, {
      type,
      localId: item?._localId as string,
    });
  ...
  pin: {
    ...
    change: (oldPin: string, newPin: string, allowAlpha: boolean) => {
      ...
      return ipcRenderer.invoke("pin:change", sessionToken, {
        oldPin,
        newPin,
        allowAlpha,
      });
  ...
  sync: {
    ...
    foldersAdd: (localPath: string, driveFolderName: string) => {
      ...
      return ipcRenderer.invoke("sync:folders:add", sessionToken, {
        localPath,
        driveFolderName,
      });
    },
    foldersRemove: (folderId: string) => {
      ...
      return ipcRenderer.invoke("sync:folders:remove", sessionToken, {
        folderId,
      });
    },
    ...
    onStatusUpdate: (cb: (config: unknown) => void) =>
      ipcRenderer.on("sync:status-update", (_e, config) => cb(config)),
  },
  ...
  minimize: () => {
    ...
    return ipcRenderer.invoke("win:minimize", sessionToken);
  },
  maximize: () => {
    bridgeLog("call", "win:maximize", true);
    return ipcRenderer.invoke("win:maximize", sessionToken);
  },
  close: () => {
    bridgeLog("call", "win:close", true);
    return ipcRenderer.invoke("win:close", sessionToken);
  },
});
